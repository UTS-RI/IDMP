...
if __name__=="__main__":
    ...
    step = 0.05
    width = 0.3
    queryGrid = np.mgrid[-width:width:step, -width:width:step, -width:width:step, 1:1.1:0.2].reshape(4,-1).T
    tPose = np.eye(4)

    while not rospy.is_shutdown():
        tPose[:3,3] = get_robot_joints_positions()[0]
        ...
        try:
            ...
            print(e)
            continue
        distsFilt = np.array(res.distances) < 0.3
